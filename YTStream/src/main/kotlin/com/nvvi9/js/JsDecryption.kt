...
internal data class JsDecryption(
    ...
    companion object {
        ...
        suspend fun fromVideoPageSource(videPageSource: String): JsDecryption? {
            return coroutineScope {
                patternDecryptionJsFile.matcher(videPageSource).takeIf { it.find() }
                    ?.group(0)?.replace("\\/", "/")?.let {
                        jsHashMap.getOrPut(it) {
                            val jsFile = RetrofitService.ytApiService.getJsFile(it)
                                .string().replace("\n", " ")
                            ...
                            if (matcher.find()) {
                                ...
                                val patternMainVariable =
                                    Pattern.compile(
                                        "(var |\\s|,|;)${decryptionFunction?.replace(
                                            "$",
                                            "\\$"
                                        )}(=function\\((.{1,3})\\)\\{)"
                                    )
                                ...
                                var mainVariable: String = if (matcher.find()) {
                                    ...
                                    val patternMainFunction =
                                        Pattern.compile(
                                            "function ${decryptionFunction?.replace(
                                                "$",
                                                "\\$"
                                            )}(\\((.{1,3})\\)\\{)"
                                        )
        ...
        private val patternFunction: Pattern = Pattern.compile("([{; =])([a-zA-Z\$_][a-zA-Z0-9$]{0,2})\\(")
        private val patternDecryptionJsFile: Pattern = Pattern.compile("\\\\/player\\\\/([^\"]+?)\\.js")
